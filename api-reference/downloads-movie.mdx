...
api: "GET https://missourimonster-vyla-v3.hf.space/api/downloads/movie/{tmdb_id}"
authMethod: "bearer"
...
const res  = await fetch('https://missourimonster-vyla-v3.hf.space/api/downloads/movie/550', {
...
data = requests.get(
  'https://missourimonster-vyla-v3.hf.space/api/downloads/movie/550',
  headers={'Authorization': 'Bearer YOUR_API_KEY'}
).json()
...
async function addMovieDownloadButtons(tmdbId, containerEl) {
  const res = await fetch(
    `https://missourimonster-vyla-v3.hf.space/api/downloads/movie/${tmdbId}`,
    { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
  );
