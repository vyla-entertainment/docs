...
api: "GET https://missourimonster-x.hf.space/api/downloads/movie/{tmdb_id}"
authMethod: "bearer"
...
<CodeGroup>
```bash cURL
curl https://missourimonster-x.hf.space/api/downloads/movie/550 \
  --header "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const res  = await fetch('https://missourimonster-x.hf.space/api/downloads/movie/550', {
...
data = requests.get(
  'https://missourimonster-x.hf.space/api/downloads/movie/550',
  headers={'Authorization': 'Bearer YOUR_API_KEY'}
).json()
...
async function addMovieDownloadButtons(tmdbId, containerEl) {
  const res = await fetch(
    `https://missourimonster-x.hf.space/api/downloads/movie/${tmdbId}`,
    { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
  );
