...
api: "GET https://api.vyla.cc/api/downloads/movie/{tmdb_id}"
authMethod: "bearer"
...
<CodeGroup>
```bash cURL
curl https://api.vyla.cc/api/downloads/movie/550 \
  --header "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const res  = await fetch('https://api.vyla.cc/api/downloads/movie/550', {
...
data = requests.get(
  'https://api.vyla.cc/api/downloads/movie/550',
  headers={'Authorization': 'Bearer YOUR_API_KEY'}
).json()
...
async function addMovieDownloadButtons(tmdbId, containerEl) {
  const res = await fetch(
    `https://api.vyla.cc/api/downloads/movie/${tmdbId}`,
    { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
  );
