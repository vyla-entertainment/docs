...
api: "GET https://missourimonster-vyla-v2.hf.space/api/downloads/movie/{tmdb_id}"
authMethod: "bearer"
...
<CodeGroup>
```bash cURL
curl https://missourimonster-vyla-v2.hf.space/api/downloads/movie/550 \
  --header "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const res  = await fetch('https://missourimonster-vyla-v2.hf.space/api/downloads/movie/550', {
...
async function addMovieDownloadButtons(tmdbId, containerEl) {
  const res = await fetch(
    `https://missourimonster-vyla-v2.hf.space/api/downloads/movie/${tmdbId}`,
    { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
  );
