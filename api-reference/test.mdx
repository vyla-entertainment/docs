...
<Note>
  This endpoint **does** run stream verification (the same `#EXTM3U` check as `/api/movie` and `/api/tv`). The `ok` field reflects whether a verified, playable URL was found — not just whether the provider returned something.
</Note>
...
<ParamField query="source" type="string" required>
  ...
  Available keys are listed in the `sources` field of [`/api/health`](https://missourimonster-vyla-api.hf.space/api/health) or [`/api/sources`](https://missourimonster-vyla-api.hf.space/api/sources).
</ParamField>
...
const res = await fetch(
  'https://missourimonster-vyla-api.hf.space/api/test/550?source=<provider-key>'
);
...
<ResponseField name="url" type="string | null">
  Proxied stream URL if the provider returned a valid stream. `null` on failure. Prepend the base URL before use.
</ResponseField>
...
async function testAll(tmdbId, season, episode) {
  ...
  const meta = await fetch(`${BASE}/api/sources`).then(r => r.json());
...
<Tip>
  For a quick overview without per-provider isolation, use [`/api/health`](https://missourimonster-vyla-api.hf.space/api/health) — it probes all providers simultaneously and is optimised for monitoring.
</Tip>