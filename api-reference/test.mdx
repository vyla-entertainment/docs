---
title: Test a Source
api: "GET https://missourimonster-vyla-v2.hf.space/api/test/{id}"
authMethod: "bearer"
...
<ParamField query="source" type="string" required>
  ...
  Available keys are listed in the `sources` field of [`/api/health`](https://missourimonster-vyla-v2.hf.space/api/health) or via [`GET /api?sources_meta=1`](https://missourimonster-vyla-v2.hf.space/api?sources_meta=1).
</ParamField>
...
```bash TV Episode (API key)
curl "https://missourimonster-vyla-v2.hf.space/api/test/1396?season=1&episode=1&source=<provider-key>" \
  --header "Authorization: Bearer YOUR_API_KEY"
```
...
const { token } = await fetch(
  'https://missourimonster-vyla-v2.hf.space/api/auth',
  { method: 'POST' }
).then(r => r.json());

const res = await fetch(
  'https://missourimonster-vyla-v2.hf.space/api/test/550?source=<provider-key>',
...
res = requests.get(
  'https://missourimonster-vyla-v2.hf.space/api/test/550',
...
<Tabs>
  <Tab title="200 — Source Working">
    ```json
    {
      ...
      "e": null,
      "ok": true,
      "url": "https://missourimonster-vyla-v2.hf.space/api?url=https%3A%2F%2F...&pa=1",
      "raw_url": "https://...",
      "elapsed_ms": 1204,
      "error": null
    }
  ...
  <Tab title="200 — TV Episode">
    ```json
    {
      ...
      "e": 1,
      "ok": true,
      "url": "https://missourimonster-vyla-v2.hf.space/api?url=https%3A%2F%2F...&pb=1",
      "raw_url": "https://...",
      "elapsed_ms": 980,
      "error": null
    }
...
<Tip>
  For a quick overview without per-provider isolation, use [`/api/health`](https://missourimonster-vyla-v2.hf.space/api/health) — it probes all providers simultaneously and is optimised for monitoring. Health accepts `public_api_key`.
</Tip>