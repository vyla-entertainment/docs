...
api: "GET https://missourimonster-vyla-v2.hf.space/api/downloads/tv/{tmdb_id}/{season}/{episode}"
authMethod: "bearer"
...
<CodeGroup>
```bash cURL
curl https://missourimonster-vyla-v2.hf.space/api/downloads/tv/1396/1/1 \
  --header "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript
const res  = await fetch(
  'https://missourimonster-vyla-v2.hf.space/api/downloads/tv/1396/1/1',
...
async function addTVDownloadButtons(tmdbId, season, episode, containerEl) {
  const res = await fetch(
    `https://missourimonster-vyla-v2.hf.space/api/downloads/tv/${tmdbId}/${season}/${episode}`,
    { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } }
  );
