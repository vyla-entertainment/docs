...
```
https://missourimonster-plsdontusethisinurprojectusetheotherone.hf.space
```
...
<Tip>
  Check which providers are currently live: [`/health`](https://missourimonster-plsdontusethisinurprojectusetheotherone.hf.space/health)
</Tip>
...
```json
{
  ...
  "source": {
    ...
    "url": "https://missourimonster-plsdontusethisinurprojectusetheotherone.hf.space/api?url=...&pp=1"
  }
}
```
...
```json
{
  "source": "provider-key",
  "label": "Provider Name",
  "url": "https://missourimonster-plsdontusethisinurprojectusetheotherone.hf.space/api?url=https%3A%2F%2F...&pp=1"
}
```
...
<ResponseField name="url" type="string">
  Fully-qualified, proxied, CORS-safe HLS stream URL. Pass directly to `hls.loadSource()` — no base URL prepending needed. All M3U8 segment and key URIs are pre-rewritten to flow through the proxy.
</ResponseField>
