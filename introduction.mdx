...
|----------|--------|---------------|-------------|
| `/api/auth` | `POST` | None | Issue a session token for browser use |
| `/movie?id=:id` | `GET` | Yes | SSE stream of sources + meta for a movie |
| `/tv?id=:id&season=:s&episode=:e` | `GET` | Yes | SSE stream of sources + meta for a TV episode |
| `/subtitles` | `GET` | None | Subtitle tracks (public) |
| `/downloads` | `GET` | None | Download links for movies or TV episodes (public) |
| `/health` | `GET` | Yes | Per-provider health check with latency |
| `/test` | `GET` | Yes | Debug a single provider in isolation |
